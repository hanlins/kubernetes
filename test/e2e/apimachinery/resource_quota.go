...
var _ = SIGDescribe("ResourceQuota", func() {
	...
	*/
	...
	ginkgo.It("should create a ResourceQuota and capture the life of a load balancer type service with allocateLoadBalancerNodePorts.", func() {
		...
		resourceQuota.Spec.Hard[v1.ResourceServicesLoadBalancers] = resource.MustParse("2")
		_, err = createResourceQuota(f.ClientSet, f.Namespace.Name, resourceQuota)
		framework.ExpectNoError(err)
		ginkgo.By("Ensuring resource quota status is calculated")
		usedResources := v1.ResourceList{}
		...
		nodeport, err = f.ClientSet.CoreV1().Services(f.Namespace.Name).Create(context.TODO(), nodeport, metav1.CreateOptions{})
		framework.ExpectNoError(err)
		ginkgo.By("Creating a LoadBalancer Service with NodePort")
		loadbalancer1 := newTestServiceForQuota("test-service-lb1", v1.ServiceTypeLoadBalancer, true)
		loadbalancer1, err = f.ClientSet.CoreV1().Services(f.Namespace.Name).Create(context.TODO(), loadbalancer1, metav1.CreateOptions{})
		framework.ExpectNoError(err)
		ginkgo.By("Not allowing a LoadBalancer Service with NodePort to be created that exceeds remaining node port quota")
		loadbalancer2 := newTestServiceForQuota("test-service-lb2", v1.ServiceTypeLoadBalancer, true)
		_, err = f.ClientSet.CoreV1().Services(f.Namespace.Name).Create(context.TODO(), loadbalancer2, metav1.CreateOptions{})
		framework.ExpectError(err)
		ginkgo.By("Creating a LoadBalancer Service without NodePort")
		loadbalancer3 := newTestServiceForQuota("test-service-lb3", v1.ServiceTypeLoadBalancer, false)
		loadbalancer3, err = f.ClientSet.CoreV1().Services(f.Namespace.Name).Create(context.TODO(), loadbalancer3, metav1.CreateOptions{})
		framework.ExpectNoError(err)
		ginkgo.By("Not allowing a LoadBalancer Service without NodePort to be created that exceeds remaining loadbalancer quota")
		loadbalancer4 := newTestServiceForQuota("test-service-lb4", v1.ServiceTypeLoadBalancer, false)
		...
		usedResources = v1.ResourceList{}
		usedResources[v1.ResourceQuotas] = resource.MustParse(strconv.Itoa(c + 1))
		usedResources[v1.ResourceServices] = resource.MustParse("4")
		// One service of NodePort and one service of LoadBalancer that allocates node port
		usedResources[v1.ResourceServicesNodePorts] = resource.MustParse("2")
		// LoadBalancer type service with and without node port allocation
		usedResources[v1.ResourceServicesLoadBalancers] = resource.MustParse("2")
		err = waitForResourceQuota(f.ClientSet, f.Namespace.Name, quotaName, usedResources)
		framework.ExpectNoError(err)
		ginkgo.By("Deleting Services")
		err = f.ClientSet.CoreV1().Services(f.Namespace.Name).Delete(context.TODO(), nodeport.Name, metav1.DeleteOptions{})
		framework.ExpectNoError(err)
		err = f.ClientSet.CoreV1().Services(f.Namespace.Name).Delete(context.TODO(), loadbalancer1.Name, metav1.DeleteOptions{})
		framework.ExpectNoError(err)
		err = f.ClientSet.CoreV1().Services(f.Namespace.Name).Delete(context.TODO(), loadbalancer3.Name, metav1.DeleteOptions{})
		framework.ExpectNoError(err)
		...
		usedResources[v1.ResourceServices] = resource.MustParse("0")
		usedResources[v1.ResourceServicesNodePorts] = resource.MustParse("0")
		usedResources[v1.ResourceServicesLoadBalancers] = resource.MustParse("0")
		err = waitForResourceQuota(f.ClientSet, f.Namespace.Name, quotaName, usedResources)
		framework.ExpectNoError(err)
	})
